...
Beacon utiliza los siguientes certificados:
...
```text
/srv/containers/bind/beacon/etc/certs/beacon_server.crt
/srv/containers/bind/beacon/etc/certs/beacon_server.key
```

Copia los certificados antes de ejecutar el script de permisos.

`deploy/fix_permissions.sh` aplica:

- modo `0644` al certificado;
- modo `0600` a la clave privada;
- ownership del UID interno de Beacon;
- etiqueta SELinux `container_file_t:s0`.

MongoDB no utiliza TLS para la comunicación interna de este despliegue.
